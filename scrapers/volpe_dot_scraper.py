"""Volpe DOT SBIR scraper.

The primary Volpe DOT SBIR page (volpe.dot.gov) is behind Cloudflare
and returns 403.  The sbir.gov API is also Cloudflare-protected.

This scraper falls back to the Grants.gov ``search2`` API filtered for
DOT (Department of Transportation) opportunities eligible for small
...
VOLPE_PAGE = "https://www.volpe.dot.gov/work-with-us/small-business-innovation-research/solicitations"

PAGE_SIZE = 100

# Search terms that capture DOT SBIR/STTR opportunities
DOT_SEARCH_TERMS = [
    "DOT SBIR transportation",
    "Federal Highway Administration SBIR",
...
class VolpeDOTScraper(BaseScraper):
    """Scraper for DOT SBIR opportunities via Grants.gov API."""

    def __init__(self):
        super().__init__("Volpe DOT SBIR")

    def fetch_data(self) -> Iterable[Dict[str, Any]]:
        """Search Grants.gov for DOT SBIR/STTR opportunities."""
        all_items: Dict[str, Dict[str, Any]] = {}

        for term in DOT_SEARCH_TERMS:
        ...
        log(f"Volpe DOT SBIR: {len(all_items)} unique DOT opportunities")
        return list(all_items.values())

    def _is_dot_agency(self, agency: str) -> bool:
        """Check if an agency string belongs to DOT."""
        agency_lower = agency.lower()
        return any(kw.lower() in agency_lower for kw in DOT_AGENCY_KEYWORDS)
    ...
    def _search_grants_gov(self, keyword: str) -> list:
        ...
        try:
            ...
            if data.get("errorcode") != 0:
                log(f"Grants.gov error for '{keyword}': {data.get('msg')}")
                return []

            opportunities = data.get("data", {}).get("oppHits", [])
            hit_count = data.get("data", {}).get("hitCount", 0)
            log(
                f"Grants.gov '{keyword}': {len(opportunities)} of "
                f"{hit_count} results"
            )
    ...
    def extract_fields(self, item: Dict[str, Any]) -> Dict[str, Any]:
        ...
        agency = item.get("agency", "DOT")

        opp_id = item.get("id", "")
        url = f"{GRANTS_GOV_DETAIL}/{opp_id}" if opp_id else VOLPE_PAGE
