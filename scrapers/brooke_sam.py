...
params = {
    ...
    "is_active": "true",
    "q": "Interceptor OR launch OR propulsion OR mobility OR 'point-to-point' OR logistics OR 'suborbital flight'",
    "qMode": "ANY",
...
def monday_create_item(title, agency, close_date_val, link, open_date_val,
    ...
    colvals = {
        ...
        LINK_COLUMN: link,
        "text_mktm7tsx": "Brooke python sam.gov",   # Source
    ...
    try:
        ...
        data = res.json()
        print(f"Created Monday item for {topic_no} ({title}) with rScore {rscore}")
        return data
    except Exception as e:
...
SLACK_CHANNEL = os.getenv("BROOKE_SLACK_CHANNEL", "")
...
def slack_bot_post_new_items(new_items):
    ...
    lines = [f"*\U0001f195 {len(new_items)} new SAM.gov opportunities for LEAP*"]
...
def slack_bot_notify_no_results(count=0):
    ...
    text = (
        f"\u2705 *SAM.gov scan completed successfully* \u2013 checked {count} opportunities for LEAP, "
